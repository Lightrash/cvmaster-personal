...
import { translatePipelineStage } from '@/lib/uiText';
...
export function DashboardPage() {
  ...
  return (
    <div className="flex-1 flex flex-col h-screen max-h-screen overflow-hidden bg-neutral-50 dark:bg-neutral-950 transition-colors duration-300">
      <PageHeader
        title="Огляд"
        subtitle="Поточний стан кандидатів, вакансій та воронки"
        ...
        <div className="h-full min-h-[620px] lg:min-h-0 grid grid-rows-[auto_minmax(0,1fr)_minmax(0,0.9fr)] gap-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <section className="rounded-xl bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 p-3.5">
              <div className="flex items-center justify-between">
                <p className="text-[11px] uppercase tracking-wide text-neutral-500">Усього кандидатів</p>
            ...
            <section className="rounded-xl bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 p-3.5">
              <div className="flex items-center justify-between">
                <p className="text-[11px] uppercase tracking-wide text-neutral-500">Відкриті вакансії</p>
            ...
            <section className="rounded-xl bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 p-3.5">
              <div className="flex items-center justify-between">
                <p className="text-[11px] uppercase tracking-wide text-neutral-500">Найнято</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[minmax(0,2fr)_minmax(260px,1fr)] gap-4 min-h-0">
            <section className="rounded-xl bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 p-4 flex flex-col min-h-0">
              <h3 className="text-[14px] font-semibold">Воронка найму</h3>
              <p className="text-[12px] text-neutral-500 mt-0.5">Поточна кількість кандидатів на кожному етапі</p>
              ...
              <div className="mt-3 flex-1 min-h-[170px] grid grid-cols-6 gap-2.5 items-end">
                {funnel.map((item) => (
                  <div key={item.status} className="h-full flex flex-col items-center justify-end gap-1.5">
                    ...
                    <div className="w-full rounded-md bg-blue-500/90" style={{ height: `${Math.max(8, (item.count / maxFunnelCount) * 100)}%` }} />
                    <span className="text-[10px] text-neutral-400 text-center leading-tight">{translatePipelineStage(item.status)}</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="rounded-xl bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 p-4 flex flex-col min-h-0">
              <h3 className="text-[14px] font-semibold">Популярні технології</h3>
              <p className="text-[12px] text-neutral-500 mt-0.5">Найчастіше зустрічаються в AI-аналізі резюме</p>

              <div className="mt-3 space-y-2 overflow-auto pr-1">
                {topTechnologies.length === 0 ? (
                  <p className="text-[12px] text-neutral-500">Даних поки немає</p>
                ) : (
                  topTechnologies.map(([tech, count], index) => (
                    <div key={tech} className="flex items-center justify-between text-[12px]">
                      <span className="text-neutral-600 dark:text-neutral-300">{index + 1}. {tech}</span>
          ...
          <section className="rounded-xl bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 overflow-hidden min-h-0 flex flex-col">
            <div className="px-4 py-2.5 border-b border-neutral-100 dark:border-neutral-800">
              <h3 className="text-[14px] font-semibold">Останні кандидати</h3>
            </div>

            {isLoading ? (
              <div className="px-4 py-5 text-[12px] text-neutral-500">Завантаження...</div>
            ) : recentCandidates.length === 0 ? (
              <div className="px-4 py-5 text-[12px] text-neutral-500">Кандидатів поки немає</div>
            ) : (
              <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
                {recentCandidates.map((candidate) => (
                  <div key={candidate.id} className="px-4 py-2.5 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-[13px] font-medium truncate">{candidate.name} {candidate.surname}</p>
                      <p className="text-[11px] text-neutral-500 truncate">{candidate.email || 'Електронну пошту не вказано'}</p>
                    </div>
                    <div className="text-[12px] text-neutral-600 dark:text-neutral-300">{candidate.position || '-'}</div>
                    <div className="text-[12px] text-neutral-600 dark:text-neutral-300">{translatePipelineStage(candidate.status)}</div>
                  </div>
                ))}
              </div>
            )}
