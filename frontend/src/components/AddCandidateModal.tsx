...
export function AddCandidateModal() {
  ...
  const handleFile = (nextFile: File) => {
    ...
    if ((ext === 'pdf' || ext === 'docx' || ext === 'doc') && nextFile.size <= 25 * 1024 * 1024) {
      setFile(nextFile);
      setError('');
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[560px] rounded-xl p-0 overflow-hidden">
        ...
        <div className="px-6 pb-5 pt-3 space-y-4">
          {!file && (
            <div
              ...
              className={`flex flex-col items-center justify-center py-10 px-6 rounded-xl border-2 border-dashed transition-all cursor-pointer group ${
                isDragOver
              ...
              <div className="w-14 h-14 rounded-xl bg-white dark:bg-neutral-800 border border-neutral-100 dark:border-neutral-700 flex items-center justify-center mb-4 shadow-sm">
                <Upload
                  className={`w-6 h-6 transition-colors ${
                    isDragOver
                      ? 'text-blue-500'
                      : 'text-neutral-300 dark:text-neutral-600 group-hover:text-blue-400'
                  }`}
                />
              </div>
